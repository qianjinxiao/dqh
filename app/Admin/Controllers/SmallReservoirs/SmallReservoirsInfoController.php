...
class SmallReservoirsInfoController extends AdminController
{
    public function index(Content $content)
    {
        ...
        $tab->add('基本信息', $this->tab("info"));
        if($ts == 1){
            $tab->add('人员岗位', $this->tab("user"),true);
        }else{
            $tab->add('人员岗位', $this->tab("user"));
        }
    ...
    public function tab($type)
    {
        ...
        SmallReservoir::GetSmallReservoir()->each(function ($item) use ($tab, $type,&$n,$ts) {
            ...
            if ($type == "info") {
                $tab->add($item->name, $this->form($item->id),$bool);
            } else {
                $tab->add($item->name, $this->grid($item->id),$bool);
            }
            $n++;
        });
